...
- `src/pages/OrcamentoForm.tsx`
  - Adicionar estado `personalizacaoFatores` (mapa nome_tabela → faixas com `quantidade_inicial`/`final`/`fator`).
...
- `src/pages/OrcamentoDetalhes.tsx:846`
  - Trocar para: `subject: \
    emailData.subject || \`Natureza Brindes - Orçamento [${orcamento.solicitacao_id}]\``.
